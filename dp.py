...
def p_state_reward(state, action):
    ...
    if action == 0:
        ...
        if next_state == end:
            reward = 100
            state = next_state
        if (next_state in diamond_block) or (next_state == start):
            state = next_state
        return((trans_pro, state, reward))
...
def compute_value_function(policy, gamma):
    ...
    theta = 0.01
    # 初始化每个状态的价值
    value_table = np.zeros(21*21)
    # 创建每次迭代更新的状态价值表
    ## 遍历所有状态
    while True:
        last_value_table = np.copy(value_table)
        for s in states:
            action = policy[states.index(s)]
            trans_prob, next_state, reward = p_state_reward(s,action)
            value_table[states.index(s)] = reward + gamma * trans_prob * value_table[states.index(next_state)]
        ## 选择当前策略下当前状态所对应的动作
        ## 返回当前状态下执行动作得到的转移概率、下一状态和奖励
        ## 计算策略下状态价值
        if np.fabs(last_value_table - value_table).max() < theta:
            break
...
def next_best_policy(value_table, gamma):
    ...
    for s in states:
        cur_value = np.zeros(4)
        ## 遍历所有动作
        for action in actions:
            trans_prob, next_state, reward = p_state_reward(s,action)
            cur_value[action] = reward + gamma * trans_prob * value_table[states.index(next_state)]
        policy.append(np.argmax(cur_value))
...
def policy_iteration(random_policy, gamma, n):
    ...
    for i in range(n):
        ## 策略评估：得到各状态的价值
        value_table = compute_value_function(random_policy, gamma)
        ## 策略提升：选取动作值最大的动作更新策略
        new_policy = next_best_policy(value_table, gamma)
        ## 对当前策略进行判断
